...
const Checkout = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Buscar o seller_id para cada item e preparar a inserção
      const itemsToInsert = await Promise.all(items.map(async (item) => {
        ...
        if (productError) throw new Error(`Erro ao buscar vendedor para o produto ${item.name}: ${productError.message}`)
      ...
      showError(error.message || 'Erro inesperado ao processar pedido')
